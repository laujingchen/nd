...
static void patch_type(Sym *sym, CType *type)
{
	...
	if (!is_compatible_types(&sym->type, type)) {
		...
		if ((type->t & VT_STATIC) && !static_proto && !(type->t & VT_INLINE))
			tcc_error("static storage ignored for redefinition of '%s'", get_tok_str(sym->v, NULL));
		...
		if ((type->t ^ sym->type.t) & VT_STATIC)
			tcc_error("storage mismatch for redefinition of '%s'", get_tok_str(sym->v, NULL));
	}
}
...
static void gen_opic(int op)
{
	...
	if (c1 && c2) {
		...
		if (!const_wanted && c1 && ((l1 == 0 && (op == TOK_SHL || op == TOK_SHR || op == TOK_SAR)) || (l1 == -1 && op == TOK_SAR))) {
			/* treat (0 << x), (0 >> x) and (-1 >> x) as constant */
			vtop--;
		} else if (!const_wanted && c2 && ((l2 == 0 && (op == '&' || op == '*')) || (op == '|' && (l2 == -1 || (l2 == 0xFFFFFFFF && t2 != VT_LLONG))) || (l2 == 1 && (op == '%' || op == TOK_UMOD)))) {
			/* treat (x & 0), (x * 0), (x | -1) and (x % 1) as constant */
			if (l2 == 1)
				vtop->c.i = 0;
			vswap();
			vtop--;
		} else if (c2 && (((op == '*' || op == '/' || op == TOK_UDIV || op == TOK_PDIV) && l2 == 1) || ((op == '+' || op == '-' || op == '|' || op == '`' || op == TOK_SHL || op == TOK_SHR || op == TOK_SAR) && l2 == 0) || (op == '&' && (l2 == -1 || (l2 == 0xFFFFFFFF && t2 != VT_LLONG))))) {
			...
			goto general_case;
		} else if (c2 && (op == '+' || op == '-') && (((vtop[-1].r & (VT_VALMASK | VT_LVAL | VT_SYM)) == (VT_CONST | VT_SYM)) || (vtop[-1].r & (VT_VALMASK | VT_LVAL)) == VT_LOCAL)) {
...
static inline int is_null_pointer(SValue *p)
{
	if ((p->r & (VT_VALMASK | VT_LVAL | VT_SYM)) != VT_CONST)
		return 0;
	return ((p->type.t & VT_BTYPE) == VT_INT && (uint32_t)p->c.i == 0) || ((p->type.t & VT_BTYPE) == VT_LLONG && p->c.i == 0) || ((p->type.t & VT_BTYPE) == VT_PTR && (PTR_SIZE == 4 ? (uint32_t)p->c.i == 0 : p->c.i == 0));
}
...
static void check_comparison_pointer_types(SValue *p1, SValue *p2, int op)
{
	...
	if ((is_integer_btype(bt1) || is_integer_btype(bt2)) && op != '-') {
		if (op != TOK_LOR && op != TOK_LAND)
			tcc_error("comparison between pointer and integer");
		return;
	}
	...
	if (!is_compatible_types(&tmp_type1, &tmp_type2)) {
		...
		if (op == '-')
			...
			tcc_error("comparison of distinct pointer types lacks a cast");
	}
}
...
redo:
	...
	if (bt1 == VT_STRUCT || bt2 == VT_STRUCT) {
		...
		if ((t1 & (VT_BTYPE | VT_UNSIGNED | VT_BITFIELD)) == (VT_LLONG | VT_UNSIGNED) || (t2 & (VT_BTYPE | VT_UNSIGNED | VT_BITFIELD)) == (VT_LLONG | VT_UNSIGNED))
			t |= VT_UNSIGNED;
		goto std_op;
	} else {
		/* integer operations */
		...
		if ((t1 & (VT_BTYPE | VT_UNSIGNED | VT_BITFIELD)) == (VT_INT | VT_UNSIGNED) || (t2 & (VT_BTYPE | VT_UNSIGNED | VT_BITFIELD)) == (VT_INT | VT_UNSIGNED))
			t |= VT_UNSIGNED;
	std_op:
		...
		if (c) {
			...
			if (df) {
				...
				if (dbt == (VT_LLONG | VT_UNSIGNED))
					...
					uint32_t m = ((dbt & VT_BTYPE) == VT_BYTE ? 0xff : (dbt & VT_BTYPE) == VT_SHORT ? 0xffff
															: 0xffffffff);
					vtop->c.i &= m;
					if (!(dbt & VT_UNSIGNED))
...
#else
			} else if ((dbt & VT_BTYPE) == VT_LLONG || (dbt & VT_BTYPE) == VT_PTR || (dbt & VT_BTYPE) == VT_FUNC) {
				if ((sbt & VT_BTYPE) != VT_LLONG && (sbt & VT_BTYPE) != VT_PTR && (sbt & VT_BTYPE) != VT_FUNC) {
...
static void gen_assign_cast(CType *dt)
{
	...
	case VT_PTR:
		...
		if (is_integer_btype(sbt)) {
			tcc_error("assignment makes pointer from integer without a cast");
			goto type_ok;
		}
		...
		if (sbt == VT_FUNC) {
			if ((type1->t & VT_BTYPE) != VT_VOID && !is_compatible_types(pointed_type(dt), st))
				tcc_error("assignment from incompatible pointer type");
			goto type_ok;
		}
		if (sbt != VT_PTR)
			goto error;
		...
		if ((type1->t & VT_BTYPE) == VT_VOID || (type2->t & VT_BTYPE) == VT_VOID) {
			...
			if (!is_compatible_unqualified_types(type1, type2)) {
				/* Like GCC don't warn by default for merely changes in pointer target signedness.
				Do warn for different base types, though, in particular for unsigned enums and signed int targets. */
		...
		if ((!(type1->t & VT_CONSTANT) && (type2->t & VT_CONSTANT))
			...
			tcc_error("assignment discards qualifiers from pointer target type");
		break;
	case VT_BYTE:
	case VT_SHORT:
	case VT_INT:
	case VT_LLONG:
		if (sbt == VT_PTR || sbt == VT_FUNC)
			tcc_error("assignment makes integer from pointer without a cast");
...
#if PTR_SIZE == 8
	#define COND (ft & VT_BTYPE) == VT_QLONG || (ft & VT_BTYPE) == VT_QFLOAT
	#define ADDRTYPE VT_LLONG
	#define LOADSIZE 8
	#define LOADTYPE (vtop->type.t & VT_BTYPE) == VT_QLONG ? VT_LLONG : VT_DOUBLE
#else
	#define COND (ft & VT_BTYPE) == VT_LLONG
	#define ADDRTYPE VT_INT
	#define LOADSIZE 4
	#define LOADTYPE VT_INT
#endif
		// MOD: Refactored this to use macros because VS Code was confused into thinking there was an extra brace
		if (COND) {
			int addr_type, load_size, load_type;
...
static Sym *find_field(CType *type, int v)
{
	...
	v |= SYM_FIELD;
	while ((s = s->next) != NULL) {
		if ((s->v & SYM_FIELD) && (s->type.t & VT_BTYPE) == VT_STRUCT && (s->v & ~SYM_FIELD) >= SYM_FIRST_ANOM) {
	...
	for (f = type->ref->next; f; f = f->next) {
		...
		if (f->v & SYM_FIRST_ANOM && (f->type.t & VT_BTYPE) == VT_STRUCT) {
	       ...
	       members in initializer handling.  */
			int v2 = f->type.ref->v;
			if (!(v2 & SYM_FIELD) && (v2 & ~SYM_STRUCT) < SYM_FIRST_ANOM) {
				Sym **pps;
		...
		if (u == VT_ENUM) {
			...
			while (tok != '}') {
				...
				while (1) {
					...
					/* If member is a struct or bit-field, enforce placing into the struct (as anonymous). */
...
/* Add type qualifiers to a type.
If the type is an array then the qualifiers are added to the element type, copied because it could be a typedef. */
...
/* return 0 if no type declaration.
otherwise, return the basic type and skip it. */
static int parse_btype(CType *type, AttributeDef *ad)
{
	int type_found, typespec_found, t, bt, st;
...
#ifdef TCC_TARGET_ARM64
		case TOK_UINT128:
			/* GCC's __uint128_t appears in some Linux header files.
			Make it a synonym for long double to get the size and alignment right. */
			u = VT_LDOUBLE;
			goto basic_type;
#endif
		case TOK_FLOAT:
...
static int post_type(CType *type, AttributeDef *ad, int storage, int td)
{
	...
	if (tok == '(') {
		...
		if (l) {
			...
			l = FUNC_OLD;
		skip(')');
		// MOD: Reaching a `)` no longer means the end of the post-declarator, there can now be `^` too
		...
		n = -1;
		if (tok != ']') {
			// MOD TODO: VLAs have been removed, this can probably be simplified further
			if (!local_stack || (storage & VT_STATIC))
				vpushi(expr_const());
			else {
				/* VLAs (which can only happen with local_stack && !VT_STATIC) length must always be evaluated, even under nocode_wanted, so that its size slot is initialized (e.g. under sizeof or typeof). */
				nocode_wanted = 0;
				gexpr();
			}
			if ((vtop->r & (VT_VALMASK | VT_LVAL | VT_SYM)) == VT_CONST) {
				...
				tcc_error("array size must be a constant");
			}
		}
		skip(']');
		...
		// MOD TODO: Handle type qualifiers
		next();
		post_type(type, ad, storage, 0);
		mk_pointer(type);
	}
...
/* Parse a type declarator (except basic type), and return the type in 'type'.
'td' is a bitmask indicating which kind of type decl is expected.
'type' should contain the basic type.
'ad' is the attribute definition of the basic type.
It can be modified by type_decl().
If this (possibly abstract) declarator is a pointer chain it returns the innermost pointed to type (equals *type, but is a different pointer), otherwise returns type itself, that's used for recursive calls. */
static CType *type_decl(CType *type, AttributeDef *ad, int *v, int td)
{
	int storage;
	CType *post, *ret;
	...
	if (tok == '(') {
		...
		if (!post_type(type, ad, 0, td)) {
			/* It's not, so it's a nested declarator, and the post operations apply to the innermost pointed to type (if any). */
...
{
	int sizeof_caller, size, align, r;
	CType type;
	Sym *s;
...
tok_next:
	...
	case '(':
		...
		if (parse_btype(&type, &ad)) {
			...
			We do retain the nocode_wanted state, as statement expressions can't ever be entered from the outside, so any reactivation of code emission (from labels or loop heads) can be disabled again after the end of it. */
			block(NULL, NULL, 1);
			nocode_wanted = saved_nocode_wanted;
			skip(')');
		} else {
		...
		break;
	// MOD TODO: `stdin` and others in stdio.h:125 use this operator
	case '&':
		next();
		unary();
	...
	case '+':
		...
		/* In order to force cast, we add zero, except for floating point where we really need an noop (otherwise -0.0 will be transformed into +0.0). */
	...
	case TOK_builtin_return_address: {
		...
		skip('(');
		if (tok != TOK_CINT)
			tcc_error("%s only takes positive integers", tok1 == TOK_builtin_return_address ? "__builtin_return_address" : "__builtin_frame_address");
	...
	case '-':
		...
		if (is_float(t)) {
			/* In IEEE negate(x) isn't subtract(0,x), but rather subtract(-0, x). */
			vpush(&vtop->type);
			if (t == VT_FLOAT)
				vtop->c.f = -1.0 * 0.0;
			else if (t == VT_DOUBLE)
	...
	case TOK_GENERIC: {
		...
		for (;;) {
			int learn, has_default, has_match;

			learn = has_default = has_match = 0;
			skip(',');
			if (tok == TOK_DEFAULT) {
				if (has_default)
	...
	tok_identifier:
		...
		if (!s || IS_ASM_SYM(s)) {
			...
			/* for simple function calls, we tolerate undeclared external reference to int() function */
			if (tcc_state->warn_implicit_function_declaration
#ifdef TCC_TARGET_PE
				/* people must be warned about using undeclared WINAPI functions (which usually start with uppercase letter) */
				|| (name[0] >= 'A' && name[0] <= 'Z')
#endif
			) {
				tcc_error("implicit declaration of function '%s'", name);
			}
		...
		Will be used by at least the x86 inline asm parser for regvars. */
		vtop->sym = s;
	...
	for (;;) {
		if (tok == TOK_INC || tok == TOK_DEC) {
			...
			/* functions names must be treated as function pointers, except for unary '&' and sizeof.
			Since we consider that functions are not lvalues, we only have to handle it there and in function calls. */
			...
			if ((s->type.t & VT_BTYPE) == VT_STRUCT) {
				...
				if (!ret_nregs) {
					...
					It is much easier to write it to memory if we know that we are allowed to write some extra bytes, so round the allocated space up to a power of 2: */
					if (size < 16)
						while (size & (size - 1))
							size = (size | (size - 1)) + 1;
#endif
			...
			if (((s->type.t & VT_BTYPE) == VT_STRUCT) && ret_nregs) {
				...
				size = type_size(&s->type, &align);
				/* We're writing whole regs often, make sure there's enough space.
				Assume register size is power of 2. */
				if (regsize > align)
					align = regsize;
				loc = (loc - size) & -align;
				addr = loc;
...
static void block(int *bsym, int *csym, int is_expr)
{
	...
	if (tok == TOK_IF) {
		...
		--local_scope;
		/* In the is_expr case (a statement expression is finished here), vtop might refer to symbols on the local_stack.
		Either via the type or via vtop->sym.
		We can't pop those nor any that in turn might be referred to.
		To make it easier we don't roll back any symbols in that case; some upper level call to block() will do that.
		We do have to remove such symbols from the lookup tables, though.
		sym_pop will do that. */
		sym_pop(&local_stack, s, is_expr);

		next();
	} else if (tok == TOK_RETURN || tok == TOK2_RET) {
		next();
		if (tok != ';') {
			gexpr();
		...
		*bsym = gjmp(*bsym);
		...
		skip(';');
		nocode_wanted |= 0x20000000;
	} else if (tok == TOK_CONTINUE || tok == TOK2_CONT) {
		/* compute jump */
		if (!csym)
		...
		*csym = gjmp(*csym);
		...
		block_after_label:
			nocode_wanted &= ~0x20000000;
			if (tok == '}') {
				tcc_error("deprecated use of label at end of compound statement");
			} else {
				if (is_expr)
					vpop();
				block(bsym, csym, is_expr);
...
static int decl_designator(CType *type, Section *sec, unsigned long c, Sym **cur_field, int size_only, int al)
{
	Sym *s, *f;
		...
		if ((vtop->r & (VT_SYM | VT_CONST)) == (VT_SYM | VT_CONST) && vtop->sym->v >= SYM_FIRST_ANOM &&
		   ...
		   until we hit one before the copied region.  */
		       ...
		       initializer is parsed.  */
					put_elf_reloca(symtab_section, sec, c + rel->r_offset - esym->st_value, ELFW(R_TYPE)(rel->r_info), ELFW(R_SYM)(rel->r_info),
#if PTR_SIZE == 8
						rel->r_addend
#else
						0
...
static void decl_initializer(CType *type, Section *sec, unsigned long c, int first, int size_only)
{
	...
	if (!have_elem && tok != '{' &&
	   ...
	   to some anonymous symbol).  */
		tok != TOK_LSTR && tok != TOK_STR && !size_only) {
		parse_init_elem(!sec ? EXPR_ANY : EXPR_CONST);
		have_elem = 1;
	}

	if (have_elem && !(type->t & VT_ARRAY) &&
...
/* 'l' is VT_LOCAL or VT_CONST to define default storage type, or VT_CMP if parsing old style parameter decl list (and FUNC_SYM is set then) */
	...
	for (;;) {
		...
		if (tok == ';') {
			if ((btype.t & VT_BTYPE) == VT_STRUCT) {
				...
				v = btype.ref->v;
				if (!(v & SYM_FIELD) && (v & ~SYM_STRUCT) >= SYM_FIRST_ANOM)
					tcc_error("unnamed struct/union that defines no instances");
				next();
				continue;
			}
			if (IS_ENUM(btype.t)) {
		...
		for (;;) {
			...
			type = btype;
			/* If the base type itself was an array type of unspecified size (like in 'typedef int arr[]; arr x = {1};') then we will overwrite the unknown size by the real one for this decl.
			...
			if (tok == '{') {
				...
				Their code will be emitted at the end of the compilation unit only if they are used */
				...
				found:
					...
					if ((type.t & VT_EXTERN && (!has_init || l != VT_CONST))
						...
						if (ad.alias_target) {
							...
							/* Local statics have a scope until now (for warnings), remove it here. */
							sym->sym_scope = 0;
