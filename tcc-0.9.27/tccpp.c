...
tail_call:
	if (!al)
		return;
	#ifdef TAL_INFO
	fprintf(stderr, "limit=%5d, size=%5g MB, nb_peak=%6d, nb_total=%8d, nb_missed=%6d, usage=%5.1f%%\n", al->limit, al->size / 1024.0 / 1024.0, al->nb_peak, al->nb_total, al->nb_missed, (al->peak_p - al->buffer) * 100.0 / al->size);
	#endif
	#ifdef TAL_DEBUG
	if (al->nb_allocs > 0) {
		uint8_t *p;
		...
		case '#':
			...
			if (start_of_line) {
				...
				if (tok == TOK_IF || tok == TOK_IFDEF || tok == TOK_IFNDEF)
					...
					a--;
				else if (tok == TOK_ERROR || tok == TOK2_ERR || tok == TOK_WARNING)
					in_warn_or_error = 1;
...
we code it as a macro to avoid pointer aliasing. */
...
static int expr_preprocess(void)
{
	...
	while (tok != TOK_LINEFEED && tok != TOK_EOF) {
		next(); /* do macro subst */
		if (tok == TOK_DEFINED || tok == TOK2_DEFINED) {
			next_nomacro();
			t = tok;
			if (t == '(')
				next_nomacro();
...
ST_FUNC void parse_define(void)
{
	...
	v = tok;
	if (v < TOK_IDENT || v == TOK_DEFINED || v == TOK2_DEFINED)
	...
	/* The body of a macro definition should be parsed such that identifiers are parsed like the file mode determines (i.e. with '.' being an ID character in asm mode).  But '#' should be retained instead of regarded as line comment leader, so still don't set ASM_FILE in parse_flags. */
	while (tok != TOK_LINEFEED && tok != TOK_EOF) {
...
static void pragma_parse(TCCState *s1)
{
	...
	if (tok == TOK_push_macro || tok == TOK_pop_macro) {
		...
		unget_tok(' ');
		// MOD TODO: `TOK2_PRAG` is not handled, but everything seems fine for now
...
static void parse_number(const char *p)
{
	...
	*q++ = t;
	b = 10;
	if (t == '.') {
		// MOD TODO: Find a proper way to disallow float literals from starting with `.`
		tcc_error("numbers cannot begin with '.'");
	} else if (t == '0') {
		if (ch == 'b') {
			--q;
			ch = *p++;
	...
	if (ch == '.'
		...
		if (b != 10) {
			...
			/* NOTE: strtox should support that for hexa numbers, but non ISOC99 libcs do not support it, so we prefer to do it by hand */
		...
		*q = '\0';
		...
		/* Determine the characteristics (unsigned and/or 64bit) the type of the constant must have according to the constant suffix(es) */
		lcount = ucount = 0;
		p1 = p;
		for (;;) {
			t = toup(ch);
	...
	const int *macro_str)
{
	...
	while (1) {
		...
		if ((t == TOK_DEFINED || t == TOK2_DEFINED) && pp_expr)
			nosubst = 2;
	}
}
